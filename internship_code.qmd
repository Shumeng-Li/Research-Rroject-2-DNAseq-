...
#### 7. Cross-referencing with COSMIC Cancer Gene Census

```{r}
#| message: false
#| warning: false
cgc_path <- "Cosmic_CancerGeneCensus_v102_GRCh38.tsv"

# Summarise mutation counts and sample coverage per gene
gene_summary <- combined_maf %>%
  dplyr::mutate(Hugo_Symbol = toupper(trimws(Hugo_Symbol))) %>%
  dplyr::filter(!is.na(Hugo_Symbol), Hugo_Symbol != "", Hugo_Symbol != "-") %>%
  dplyr::group_by(Hugo_Symbol) %>%
  dplyr::summarise(
    mutation_count  = dplyr::n(),
    samples_mutated = dplyr::n_distinct(Tumor_Sample_Barcode),
    variant_classes = paste0(sort(unique(Variant_Classification)), collapse = ";"),
    .groups = "drop"
  ) %>%
  dplyr::arrange(dplyr::desc(mutation_count))

# Read COSMIC CGC file
cgc_raw <- readr::read_tsv(cgc_path, guess_max = 10000, show_col_types = FALSE)
cgc_cols_lower <- tolower(names(cgc_raw))

sym_col <- names(cgc_raw)[match(TRUE, cgc_cols_lower %in% c("gene symbol","gene_symbol","symbol","gene"))]
if (is.na(sym_col)) stop("Could not detect gene symbol column in CGC file. Please check TSV headers.")

tier_col <- names(cgc_raw)[match(TRUE, cgc_cols_lower %in% c("tier"))]
som_col <- names(cgc_raw)[match(TRUE, cgc_cols_lower %in% c("somatic"))]
hallmark_col <- names(cgc_raw)[match(TRUE, cgc_cols_lower %in% c("hallmark","hallmarks","hallmark of cancer","hallmarks of cancer"))]
tumourtype_col<- names(cgc_raw)[match(TRUE, grepl("tumour|tumor", cgc_cols_lower) & grepl("type", cgc_cols_lower))]

cgc_df <- cgc_raw %>%
  dplyr::transmute(
    CGC_Symbol     = toupper(trimws(.data[[sym_col]])),
    CGC_Tier       = if (!is.na(tier_col))      as.character(.data[[tier_col]])      else NA_character_,
    CGC_Somatic    = if (!is.na(som_col))       as.character(.data[[som_col]])       else NA_character_,
    CGC_Hallmark   = if (!is.na(hallmark_col))  as.character(.data[[hallmark_col]])  else NA_character_,
    CGC_TumourType = if (!is.na(tumourtype_col))as.character(.data[[tumourtype_col]])else NA_character_
  ) %>% dplyr::distinct(CGC_Symbol, .keep_all = TRUE)

# Cross-reference: mark genes that are in the CGC list
cross_all <- gene_summary %>%
  dplyr::left_join(cgc_df, by = c("Hugo_Symbol" = "CGC_Symbol")) %>%
  dplyr::mutate(is_CGC = !is.na(CGC_Tier) | !is.na(CGC_Somatic) | !is.na(CGC_Hallmark))

# Save all cross-reference results and CGC-only summary
readr::write_tsv(cross_all, file.path(output_dir, "cgc_cross_reference_all_genes.tsv"))

cgc_hits <- cross_all %>%
  dplyr::filter(is_CGC) %>%
  dplyr::arrange(dplyr::desc(mutation_count)) %>%
  dplyr::select(Hugo_Symbol, mutation_count, samples_mutated,
                CGC_Tier, CGC_Somatic, CGC_Hallmark, CGC_TumourType, variant_classes)
readr::write_tsv(cgc_hits, file.path(output_dir, "cgc_driver_genes_summary.tsv"))
```

```{r}
#| message: false
#| warning: false
# Plot top N genes highlighting CGC vs non-CGC status
top_n <- 20
plot_df <- cross_all %>%
  dplyr::arrange(dplyr::desc(mutation_count)) %>%
  dplyr::slice_head(n = top_n) %>%
  dplyr::mutate(CGC_Status = ifelse(is_CGC, "CGC", "Non-CGC"),
                label = paste0(Hugo_Symbol, " (", samples_mutated, ")"))

p <- ggplot2::ggplot(plot_df,
                     ggplot2::aes(x = reorder(label, mutation_count),
                                  y = mutation_count, fill = CGC_Status)) +
  ggplot2::geom_col() +
  ggplot2::coord_flip() +
  ggplot2::labs(title = paste0("Top ", top_n, " mutated genes (CGC vs Non-CGC)"),
                x = "Gene (samples mutated)", y = "Mutation count") +
  ggplot2::theme_minimal(base_size = 11) +
  ggplot2::theme(legend.position = "top")

ggplot2::ggsave(file.path(output_dir_p1_fig, "top_mutated_genes_CGC_vs_nonCGC.png"),
                p, width = 7.5, height = 6, dpi = 300, bg = "white")

# 5) Summary
cat(sprintf("Total unique genes in MAF: %d\n", nrow(gene_summary)))
cat(sprintf("CGC genes detected in cohort: %d\n", nrow(cgc_hits)))
print(utils::head(cgc_hits, 10))
p
```
